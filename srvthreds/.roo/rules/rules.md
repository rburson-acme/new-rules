...
npm run bootstrap          # Set up data and configurations for running servers
```
...
- Agents may run as trusted and co-resident (agent.ts, AgentService.ts) or remote (remote-agent.ts, RemoteAgentService.ts)
...
### Adding New Patterns
1. Create test patterns JSON in `src/ts/config/patterns/`
