...
export class AKSDeployer extends BaseDeployer {
  ...
  constructor(options: AKSDeployerOptions) {
    ...
    this.aksOptions = {
      ...
      manifestPath: options.manifestPath || `infrastructure/cloud/kubernetes/manifests/${options.environment}/`,
  ...
  protected async buildImages(): Promise<void> {
    ...
    try {
      ...
      await this.shell.exec('tsx', ['infrastructure/tools/shared/config/generator.ts']);

      this.logger.info('Validating configuration consistency...');
      await this.shell.exec('tsx', ['infrastructure/tools/shared/config/validator.ts']);

      // Create empty .env placeholder (Kubernetes uses ConfigMap instead)
      this.logger.info('Creating deployment assets...');
      await this.shell.exec('touch', ['./infrastructure/tools/deployment-cli/assets/.env']);
      ...
      // Cleanup assets
      this.logger.info('Cleaning up deployment assets...');
      await this.shell.exec('find', [
